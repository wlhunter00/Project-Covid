...
export function SymptomsList({ navigation }) {
  const { styles, colors, isDark } = useStyle("sectionTitle", "container", "boxContainer", "surveyQuestionText", "scrollViewContent", "compactNameSmall", "divider");
  ...
  // Load the symptoms
  React.useEffect(() => {
    const symptoms = symptomData.map(symptom => ({ title: symptom.Symptom, percentage: symptom.Percentage }));
  ...
  const symptomsToDisplay = !isSearching ? allSymptoms : allSymptoms.filter(symptom => symptom.title.toLowerCase().includes(currentSearchQuery.toLowerCase()));
  ...
  return (
    <View style={[styles.container]}>
      <ParallaxScrollView
        ...
        renderForeground={() => (
          <View style={{ marginBottom: 20, marginTop: 100, paddingHorizontal: 15, flexDirection: "row" }}>
            <View style={{ flex: 1 }}>
            ...
            <FontAwesome name="stethoscope" size={100} color={!isDark ? colors.textcolor : "#444"} />
          </View>
        )}
        renderStickyHeader={() => (
          <View style={{ borderBottomColor: colors.accentcolor, borderBottomWidth: 1, paddingHorizontal: 15, paddingBottom: 12, justifyContent: 'flex-end', height: '100%' }}>
      ...
      >
        ...
        <StandardText style={{ textAlign: "right", color: colors.secondarytextcolor }}>{symptomsToDisplay.length} item(s) found</StandardText>
